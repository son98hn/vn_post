	...
	<%@include file="/common/taglib.jsp" %>
		...
		<body>
			<div class="main-content">
				<form action="<c:url value='/admin-grouprole'/>" id="formSubmit" method="GET">
											...
											<div class="pull-right tableTools-container">
												<div class="dt-buttons btn-overlap btn-group">
													...
													<a flag="info" class="dt-button buttons-colvis btn btn-white btn-primary
														btn-bold" data-toggle="tooltip" title='Thêm nhóm quyền' href='
														<c:url value="/admin-grouprole?type=edit" />' <c:if
														test="${userModel.listRoleDetailCode.indexOf('add-grouprole') == -1}">
														style="visibility: hidden;"</c:if>> <span>
															<i class="fa fa-plus-circle bigger-110 purple"></i>
														</span>
													</a>
													<button id="btnDelete" type="button" <c:if
														test="${userModel.listRoleDetailCode.indexOf('delete-grouprole') == -1}">
														style="visibility: hidden;"</c:if>
														class="dt-button buttons-html5 btn btn-white btn-primary
														btn-bold"
														data-toggle="tooltip" title='Xóa nhóm quyền'>
													...
													<tbody>
														<c:forEach var="item" items="${model.listResult}" varStatus="loop">
															<tr>
																...
																<td>
																		${model.listRoleDetailName[loop.index]}
																</td>
																<td>
																	<c:url var="editURL" value="/admin-grouprole">
																		<c:param name="type" value="edit" />
																		<c:param name="id" value="${item.id}" />
																	</c:url> <a class="btn btn-sm btn-primary btn-edit"
																		data-toggle="tooltip"
																		title="Cập nhật nhóm quyền" <c:if
																		test="${userModel.listRoleDetailCode.indexOf('edit-grouprole') == -1}">
																		style="visibility: hidden;"</c:if>
																		href='${editURL}'><i
					...
					$
						.ajax({
							...
							success: function (result) {
								window.location.href = "${GroupRoleURL}?type=list&maxPageItem=6&page=1&message=delete_success";
							},
